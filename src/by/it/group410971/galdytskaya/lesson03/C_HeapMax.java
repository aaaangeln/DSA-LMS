...
public class C_HeapMax {
    ...
    private class MaxHeap {
        ...
        int siftDown(int i) { // просеивания вниз
            int left = 2 * i + 1;
            int right = 2 * i + 2;
        ...
        int siftUp(int i) { // просеивания вверх
            while (i > 0) {
                int parent = (i - 1) / 2;
                // если родительский элемент меньше текущего
        ...
        Long extractMax() { //извлечение и удаление максимума
            ...
            Long max = heap.get(0); // Сохраняем максимальный элемент
            Long last = heap.remove(heap.size() - 1); // Удаляем последний элемент
            if (!heap.isEmpty()) {
                heap.set(0, last); // Перемещаем последний элемент на корень
