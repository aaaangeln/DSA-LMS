...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        Map<Character, Integer> count = new HashMap<>(); // переберем все символы по очереди и рассчитаем их частоту в Map count
        for (char c : s.toCharArray()) {
            count.put(c, count.getOrDefault(c, 0) + 1);
        }

        PriorityQueue<Node> priorityQueue = new PriorityQueue<>(); //перенесем все символы в приоритетную очередь в виде листьев
        for (Map.Entry<Character, Integer> entry : count.entrySet()) {
            priorityQueue.add(new LeafNode(entry.getValue(), entry.getKey()));
        }

        while (priorityQueue.size() > 1) { // вынимая по два узла из очереди, строитм дерево
        ...
        Node root = priorityQueue.poll(); // последний из родителей будет корнем этого дерева
        root.fillCodes("");

        StringBuilder sb = new StringBuilder(); // кодировка
        for (char c : s.toCharArray()) {
